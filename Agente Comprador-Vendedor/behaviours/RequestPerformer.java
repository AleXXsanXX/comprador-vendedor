...
public class RequestPerformer extends Behaviour{
    ...
    case 3:      
        ...
        if (reply != null) {
            if (reply.getPerformative() == ACLMessage.INFORM) {
                ...
                // Ejecuta un dialog que muestra al usuario los datos de la
                // compra.
                bbAgent.getGUI().showNewSeller(bookTitle,
                        reply.getSender().getName(), bestPrice);

                System.out.printf(
                        "\"%s\" successfully purchased from agent \"%s\"\n",
                        bookTitle,
  ...
  public boolean done() {
    if (step == 2 && bestSeller == null) {
       System.out.printf("Attempt failed: \"%s\" not available for sale\n",
               bookTitle);
    }
